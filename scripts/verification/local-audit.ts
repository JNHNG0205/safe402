...
const USAGE = 'usage: local-audit <fixture-dir> [--policy file] [--json]';
const DEFAULT_POLICY = 'config/policies/research-agent.yaml';
const args = process.argv.slice(2);
const json = args.includes('--json');
const log = (m: string) => console.error(m);
/** Prints the machine-readable form when asked and returns the exit code; the caller sets it. */
const finish = (code: number, out: unknown): number => { if (json) console.log(JSON.stringify(out, null, 2)); return code; };

async function main(): Promise<number> {
  const policyIdx = args.indexOf('--policy');
  if (policyIdx >= 0 && (args[policyIdx + 1] === undefined || args[policyIdx + 1]!.startsWith('--'))) {
    log(`[local-audit] --policy needs a file path`); log(USAGE);
    return finish(6, { error: '--policy needs a file path' });
  }
  const policyPath = policyIdx >= 0 ? args[policyIdx + 1]! : DEFAULT_POLICY;
  const fixture = args.find((a) => !a.startsWith('--') && a !== policyPath);
  if (!fixture) { log(`[local-audit] ${USAGE}`); return finish(6, { error: USAGE }); }
  ...
  try {
    ...
    if (!existsSync(resolve(policyPath))) throw new PolicyError(`policy file not found: ${policyPath}`);
    ...
    if (final.status !== 'COMPLETED') {
      ...
      return finish(5, { ...out, failure: dependency ? 'DEPENDENCY_UNAVAILABLE' : 'JOB_FAILED' });
    }
    return finish(out.decision === 'ALLOW' ? 0 : out.decision === 'BLOCK' ? 2 : 3, out);
  } catch (e) {
    ...
    return finish(badInput ? 6 : 5, { error: err.message, failure: badInput ? 'BAD_INPUT' : err instanceof DependencyUnavailable ? 'DEPENDENCY_UNAVAILABLE' : 'ERROR' });
  }
}
